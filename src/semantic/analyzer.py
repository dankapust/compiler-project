...
class SemanticAnalyzer:
    ...
    def _analyze_global_var(self, node: VarDeclStmt) -> None:
        ...
        info = SymbolInfo(
            ...
            line=node.line,
            column=node.column,
            # Важно: пока не вычислили initializer, переменная считается НЕ инициализированной.
            # Это позволяет корректно ловить use-before-init (в т.ч. самоиспользование в своём initializer).
            initialized=False,
            stack_offset=None,
        )
    ...
    def _analyze_var_decl(self, node: VarDeclStmt) -> None:
        ...
        info = SymbolInfo(
            ...
            line=node.line,
            column=node.column,
            # См. комментарий в _analyze_global_var: при наличии initializer переменная
            # считается не инициализированной до вычисления выражения.
            initialized=False,
            stack_offset=off,
            size_bytes=sz,
            alignment=al,
        ...
        match node:
            ...
            case AssignmentExpr(target=target, operator=op, value=value):
                ...
                if op == "=":
                    ...
                    elif sym.type.kind == TypeKind.INT and vt.kind == TypeKind.FLOAT:
                        self._err(
                            "type_mismatch",
                ...
                sym.initialized = True
                self._expr_types[id(node)] = sym.type
                return sym.type
            case IncDecExpr(target=name, operator=op):
                sym = self._table.lookup(name)
                if sym is None:
                    self._err(
                        "undeclared_identifier",
                ...
                if sym.kind != SymbolKind.VARIABLE:
                    self._err(
                        "invalid_assignment_target",
                        f"цель для ++/-- должна быть переменной",
                        node.line,
                        node.column,
                    )
                    return ERROR_T
                self._symbol_refs[id(node)] = sym
                if not is_numeric(sym.type):
                    self._err(
                        "type_mismatch",
