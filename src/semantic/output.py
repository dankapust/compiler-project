...
def format_validation_report(
    ...
    lines.append("")
    lines.append("Иерархия типов (summary):")
    lines.append("  Базовые: int, float, bool, void, string, null")
    struct_lines: list[str] = []
    fn_lines: list[str] = []
    for _, syms in table.dump_scopes():
        for _, info in syms.items():
            if info.kind == SymbolKind.STRUCT and info.struct_fields:
                fields = ", ".join(f"{n}:{t}" for n, t in info.struct_fields.items())
                struct_lines.append(f"    {info.name} {{ {fields} }}")
            elif info.kind == SymbolKind.FUNCTION and info.param_types is not None:
                ps = ", ".join(str(t) for t in info.param_types)
                rt = str(info.return_type) if info.return_type else "void"
                fn_lines.append(f"    {info.name}: fn({ps}) -> {rt}")
    lines.append("  Struct:")
